...
export const useBreakitModesStore = create<BreakitModesState>()(
  persist(
    (set, get) => ({
      selectedMode: null,
      selectedCharts: [],
      ...
      setSelectedMode: (mode: string | null) => {
        ...
        const updates: Partial<BreakitModesState> = { selectedMode: mode };
...
export function useBreakitModePersistence() {
  const store = useBreakitModesStore();
