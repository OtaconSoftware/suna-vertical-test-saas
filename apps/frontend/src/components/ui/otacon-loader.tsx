...
/**
 * OtaconLoader - A unified circular loading animation component
 * 
 ...
 * <OtaconLoader variant="white" />
 * 
 * // Always black (for light backgrounds in any theme)
 * <OtaconLoader variant="black" />
 * 
 * // Custom size
 * <OtaconLoader size="large" />
 * ```
 */
export function OtaconLoader({
  size = 'medium',
  speed = 1.2,
  customSize,
  className,
      ...
      <div
        style={{
          ...
          animation: autoPlay && loop 
            ? `otacon-spin ${animationDuration}s linear infinite` 
            : autoPlay 
              ? `otacon-spin ${animationDuration}s linear` 
              : 'none',
        }}
      />
      <style jsx>{`
        @keyframes otacon-spin {
          to {
