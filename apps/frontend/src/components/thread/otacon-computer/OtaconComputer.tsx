...
import { 
  useOtaconComputerStore,
  useOtaconComputerPendingToolNavIndex,
  useOtaconComputerClearPendingToolNav,
} from '@/stores/otacon-computer-store';
...
const FLOATING_LAYOUT_ID = 'otacon-computer-float';

export const OtaconComputer = memo(function OtaconComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useOtaconComputerPendingToolNavIndex();
  const clearPendingToolNav = useOtaconComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[OtaconComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the Otacon Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the Otacon Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="otacon-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
