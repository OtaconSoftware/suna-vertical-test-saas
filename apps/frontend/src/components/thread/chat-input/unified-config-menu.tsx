...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const breakitAgent = useMemo(() => {
        return agents.find(a => a.metadata?.is_breakit_default === true);
    }, [agents]);

    const placeholderBreakitAgent = useMemo(() => ({
        agent_id: undefined,
        name: 'Otacon',
        metadata: { is_breakit_default: true }
    }), []);
    ...
    const displayAgent = useMemo(() => {
        ...
        if (breakitAgent) return breakitAgent;
        return agents[0];
    }, [agents, selectedAgentId, breakitAgent]);
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || breakitAgent)) {
            return <AgentAvatar isBreakitDefault={true} agentName="Otacon" size={size} className="flex-shrink-0 !border-0" />;
        }
        return <AgentAvatar agent={agent} agentId={agent?.agent_id} size={size} className="flex-shrink-0 !border-0" />;
    }, [isLoading, breakitAgent]);
    ...
    // Check if current agent is Otacon (used in multiple places)
    const isOtaconAgent = !displayAgent?.name || displayAgent?.name === 'Otacon';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                {isOtaconAgent ? (
                                    ...
                                    <div className="flex items-center justify-center w-10 h-10 bg-card border-[1.5px] border-border flex-shrink-0" style={{ borderRadius: '10.4px' }}>
                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderBreakitAgent : displayAgent, 40)}
                                    </div>
                                )}
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isOtaconAgent ? 'Otacon' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderBreakitAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isOtaconAgent]);
        ...
        >
            {onAgentSelect ? (
                <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                    {renderAgentIcon(isLoading && !displayAgent ? placeholderBreakitAgent : displayAgent, 24)}
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isOtaconAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                                                <DropdownMenuSubTrigger className="flex items-center gap-3 text-sm cursor-pointer px-1 hover:bg-transparent focus:bg-transparent data-[state=open]:bg-transparent w-full">
                                                    <div className="flex items-center gap-3">
                                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderBreakitAgent : displayAgent)}
                ...
                <span className="inline-block">
                    ...
                    >
                        <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                            ...
                            <span className="truncate text-sm font-medium">Otacon</span>
                            <ChevronDown size={12} className="opacity-60 flex-shrink-0" />
                        </div>
                    </Button>
                </span>
