...
import { ToolCallInput } from '@/components/thread/otacon-computer';
...
import { useOtaconComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/otacon-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useOtaconComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure OtaconComputer switches to tools view
