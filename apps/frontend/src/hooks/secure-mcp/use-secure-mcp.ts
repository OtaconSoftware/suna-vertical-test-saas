...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_otacon_team !== undefined) searchParams.set('is_otacon_team', params.is_otacon_team.toString());
...
export function useOtaconTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'otacon-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view Otacon templates');
      }

      const response = await fetch(`${API_URL}/templates/otacon-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
