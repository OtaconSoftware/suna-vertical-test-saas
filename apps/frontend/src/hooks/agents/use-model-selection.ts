...
const getDefaultModel = (accessibleModels: ModelOption[]): string => {
  ...
  // otacon/basic should be first for free users since power is not accessible
  const basicModel = accessibleModels.find(m => m.id === 'otacon/basic');
  if (basicModel) return basicModel.id;

  const powerModel = accessibleModels.find(m => m.id === 'otacon/power');
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      const powerModel = availableModels.find(m => m.id === 'otacon/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to otacon/power');
        setSelectedModel('otacon/power');
      }
    }
